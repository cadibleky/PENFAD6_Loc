...
{
    public class Report_SchemeBenefitController : Controller
    {
        // GET:
        cLogger logger = new cLogger();
        readonly pfm_SchemeRepo schemeRepo = new pfm_SchemeRepo();

        public ActionResult Report_SchemeBenefit_Tab(string containerId = "MainArea")
        {
        ...
        {
            return PartialView("Report_SchemeBenefitPartial");
        }

        public ActionResult Load_Employee_Report_IFrame(string containerId, string Scheme_Id, string tree_node_id, DateTime? TB_Date)
        {
            try
            {
                ...
                GlobalValue.Report_Param_1_date = TB_Date;
                GlobalValue.Report_Index_Id = tree_node_id;

                //load report table
                var con = new AppSettings();
                var param = new DynamicParameters();
                param.Add("P_SCHEME_ID", Scheme_Id, DbType.String, ParameterDirection.Input);
                param.Add("P_MAKER_ID", GlobalValue.User_ID, DbType.String, ParameterDirection.Input);
                param.Add("P_MAKE_DATE", TB_Date, DbType.DateTime, ParameterDirection.Input);
                con.GetConnection().Execute("REPORT_SCHEME_STATEMENT", param, commandType: CommandType.StoredProcedure);
