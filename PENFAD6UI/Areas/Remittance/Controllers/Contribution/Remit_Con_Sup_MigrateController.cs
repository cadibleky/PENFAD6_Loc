...
namespace PENFAD6UI.Areas.Remittance.Controllers.Contribution
{
    ...
    public class Remit_Con_Sup_MigrateController : Controller
    {
        string clog;
        string plog;
        ...
        public ActionResult AddConSupMigrationTab(string containerId = "MainArea")
        {
            var pvr = new Ext.Net.MVC.PartialViewResult
            {
                ViewName = "Remit_Con_Sup_MigrationPartial",
        ...
        public ActionResult AddConBPayMigrationTab(string containerId = "MainArea")
        {
            var pvr = new Ext.Net.MVC.PartialViewResult
            {
                ViewName = "Remit_Con_BPay_MigrationPartial",
                ContainerId = containerId,
                RenderMode = RenderMode.AddTo,
            };
            X.Mask.Hide();
        ...
        public ActionResult RemitConMigrationUpload(Remit_Con_Log remit_con_logrepo)
        {
            try
            {
                ...
                string query2 = "Select * From [SalaryData3$]";
                ...
                if (srda.HasRows)
                {
                        ...
                        i++)
                    {
                            ...
                            .Any(row => row["ColumnName"].ToString() == con_date_string))
                        {
                            ...
                            string conlog = "CON" + remit_con_logrepo.ES_Id + Convert.ToInt32(con_date.ToString("yyyy")) + clog + "10";
                    ...
                    while (srda.Read())
                    {
                        ...
                        foreach (var conlogcols in remitConLogstaticlist)
                        {
                            ...
                            if (srda[con_date_str] != DBNull.Value && srda[con_date_str].ToString() != "-" && Convert.ToDecimal(srda[con_date_str]) != 0 )
                            {
                                ...
                                //new_conlogdetails_Newest.Trustee_Name = srda["TrusteeName"].ToString();
                                new_conlogdetails_Newest.Employee_Con = Convert.ToDecimal(srda[con_date_str]);
                                new_conlogdetails_Newest.Con_Log_Id = "CON" + conlogcols.ES_Id + conlogcols.For_Year + plog + "11";
                                new_conlogdetails_Newest.Esf_Id = new_EsfID;
                                new_conlogdetails_Newest.Employee_Amt = Convert.ToDecimal(srda[con_date_str]);
                                new_conlogdetails_Newest.Purchase_Log_Id = remit_con_logrepo.ES_Id + conlogcols.For_Year + plog + "11";
                ...
                using (TransactionScope transcope = new TransactionScope(TransactionScopeOption.Required, new System.TimeSpan(1, 30, 0)))
                {
                    ...
                    foreach (var conlogcols in remitConLogstaticlist)
                    {
                        string cnl = remitConLogrepo.Create_Con_Log2(conlogcols);  
                    }

                    //create purchase log
                    foreach (var conlogcols in remitConLogstaticlist)
                    {
                        string cnl = remitConLogrepo.PurchaseSaveRecord2(conlogcols);
                    }
                    ...
                    {
                        remitConLogdetailsrepo.Create_Con_Log_Details2(conlogcolsdetails);
                    }

                    //Push Contribution into purchase trans table
                    foreach (var conlogcolsdetails in remitConLogDetailsStaticlist)
                    {
                        remitConLogdetailsrepo.Create_Unit_Log_Details2(conlogcolsdetails);
                    }                 
                ...
                {
                    Title = "Success",
                    Message = "Transfer In history remittances uploaded successfully",
                    Buttons = MessageBox.Button.OK,
                    Icon = MessageBox.Icon.INFO,
                    Width = 350
                });
            ...
            catch (Exception ex)
            {
                ...
                {
                    ora_code = "PROCESS ABORTED. UNEXPECTED ERROR";
                }
                else
                {
        ...
        // upload for back pay
        public ActionResult RemitConMigrationUploadBP(Remit_Con_Log remit_con_logrepo)
        {
            try
            {
                ...
                //string query1 = "Select COUNT(*) AS NOS From [SalaryData$]";
                string query2 = "Select * From [SalaryData33$]";
                    ...
                    while (srda.Read())
                    {
                        ...
                        foreach (var conlogcols in remitConLogstaticlist)
                        {
                            ...
                            if (srda[con_date_str] != DBNull.Value && srda[con_date_str].ToString() != "-" && Convert.ToDecimal(srda[con_date_str]) != 0)
                            {
                              ...
                              //  new_conlogdetails_Newest.Trustee_Name = srda["TrusteeName"].ToString();
                                new_conlogdetails_Newest.Employee_Con = Convert.ToDecimal(srda[con_date_str]);
                                new_conlogdetails_Newest.Con_Log_Id = "CON" + conlogcols.ES_Id + conlogcols.For_Year + plog + "05";
                                new_conlogdetails_Newest.Esf_Id = new_EsfID;
                                new_conlogdetails_Newest.Employee_Amt = Convert.ToDecimal(srda[con_date_str]);
                ...
                using (TransactionScope transcope = new TransactionScope(TransactionScopeOption.Required, new System.TimeSpan(1, 30, 0)))
                {
                    ...
                    foreach (var conlogcols in remitConLogstaticlist)
                    {
                        string cnl = remitConLogrepo.Create_Con_Log_BP(conlogcols);
                    }

                    //create purchase log
                    foreach (var conlogcols in remitConLogstaticlist)
                    {
                        string cnl = remitConLogrepo.PurchaseSaveRecord_BP(conlogcols);
                    }
                    ...
                    foreach (var conlogcolsdetails in remitConLogDetailsStaticlist)
                    {
                        remitConLogdetailsrepo.Create_Con_Log_Details_BP(conlogcolsdetails);
                    }

                    //Push Contribution into purchase trans table
                    foreach (var conlogcolsdetails in remitConLogDetailsStaticlist)
                    {
                        remitConLogdetailsrepo.Create_Unit_Log_Details_BP(conlogcolsdetails);
                    }
                ...
                {
                    Title = "Success",
                    Message = "Back Pay remittances uploaded successfully",
                    Buttons = MessageBox.Button.OK,
                    Icon = MessageBox.Icon.INFO,
                    Width = 350
                });
