...
{
    public class Remit_ContributionBackUpLoad_LogController : Controller
    {
        string plog;
        ...
        public ActionResult AddContributionRemitTab_Back(string containerId = "MainArea")
        {
            var pvr = new Ext.Net.MVC.PartialViewResult
            {
                ViewName = "ContributionRemit_BackPartial",
        ...
        public ActionResult GetMDate(Remit_Contribution_Supplimentary_LogRepo RemitInitialRepo)
        {
            try
            {
                ...
                dd = dd.AddMonths(1);
                this.GetCmp<DateField>("txtDeadline_Date_Back").SetValue(dd);              
                return this.Direct();
            }
            catch (Exception ex)
            {
        ...
        public ActionResult ClearControls()
        {
            try
            {
                var x = X.GetCmp<FormPanel>("frm_remittanceContribution_Back");
                x.Reset();
                return this.Direct();
            }
        ...
        public ActionResult UploadRemittanceClick(Remit_Contribution_Upload_LogRepo remit_cont_repo, string param_employer_scheme_id, string Month_str, string year_str)
        {
            ...
            {
                //Check if file Exist
                if (this.GetCmp<FileUploadField>("remitfile_upload1_Back").HasFile)
                {
                    HttpPostedFile file_posted = this.GetCmp<FileUploadField>("remitfile_upload1_Back").PostedFile;
                ...
                else
                {
                    ...
                    Store store = X.GetCmp<Store>("employerscheme_store_Back");
                    store.Reload();
                        ...
                        using (OracleConnection conn = new OracleConnection(app.conString()))  //
                        {
                            ...
                            try
                            {
                               ...
                               // string batchno = "";
                                ...
                                conn.Execute(sql: "ADD_REMIT_CON_LOG_BACK", param: paramb, commandType: CommandType.StoredProcedure);
                                ...
                                while (srda.Read())
                                {
                                    ...
                                    if (create_new_or_justacct == 1)
                                    {
                                        ...
                                        param_ka.Add(name: "p_contype", value: "BACKPAY-CONTRIBUTION", dbType: DbType.String, direction: ParameterDirection.Input);
