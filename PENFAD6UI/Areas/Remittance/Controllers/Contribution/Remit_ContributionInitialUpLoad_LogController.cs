...
{
    public class Remit_ContributionInitialUpLoad_LogController : Controller
    {
        string plog;
        ...
        public ActionResult AddContributionRemitTab(string containerId = "MainArea")
        {
            var pvr = new Ext.Net.MVC.PartialViewResult
            {
                ViewName = "ContributionRemitPartial",
        ...
        public ActionResult GetMDate(Remit_Contribution_Upload_LogRepo RemitInitialRepo)
        {
            try
            {
                ...
                dd = dd.AddMonths(1);
                this.GetCmp<DateField>("txtDeadline_Date").SetValue(dd);
                return this.Direct();
            }
            catch (Exception ex)
            {
        ...
        public ActionResult Get_Month_Year_For_Employer_Upload(string param_employer_scheme_id, string param_scheme_id, Remit_Contribution_Upload_LogRepo RemitInitialRepo)
        {
            ...
            try
            {
                ...
                {
                    var cmd_month = X.GetCmp<ComboBox>("ContributionRemitPartial_txtremittancemonth");
                    cmd_month.Enable();
                    var cmd_year = X.GetCmp<ComboBox>("ContributionRemitPartial_txtfrmRemittance_year");
                    cmd_year.Enable();
                }
                else
                {
                    ...
                    int yr = Convert .ToInt32(year_r);
                    int mt = Convert.ToInt32(mon_th);

                    DateTime DLD = new DateTime(yr, mt, 14);
                     DLD = DLD.AddMonths(1);
                    this.GetCmp<DateField>("txtDeadline_Date").SetValue(DLD);
                ...
                //    //DateTime? next_deadline_date = Convert.ToDateTime(dead_line_day.ToString() + "/" + mon_th.ToString() + "/" + year_r.ToString());
                //    //if (next_deadline_date < scheme_today_date)
                //    //{
                //    //    next_deadline_date = scheme_deadLine_date;
                //    //}

                //    var cmd_deadlineDate = X.GetCmp<TextField>("txtDeadline_Date");
        ...
        public ActionResult ClearControls()
        {
            try
            {
                var x = X.GetCmp<FormPanel>("frm_remittanceContribution");
                x.Reset();

                /// this.GetCmp<Store>("employerscheme_store").Reload();
                ///  this.GetCmp<Store>("emp_list_store").Reload();
                return this.Direct();
            }
        ...
        public ActionResult UploadRemittanceClick(Remit_Contribution_Upload_LogRepo remit_cont_repo, string param_employer_scheme_id, string Month_str, string year_str)
        {
            ...
            try
            {
                ...
                //Check if file Exist
                if (this.GetCmp<FileUploadField>("remitfile_upload1").HasFile)
                {
                    HttpPostedFile file_posted = this.GetCmp<FileUploadField>("remitfile_upload1").PostedFile;
                    ...
                    if (Month_str != remit_cont_repo.For_Month.ToString() && year_str != remit_cont_repo.For_Year.ToString())
                    {
                        ...
                        {
                            Title = "Error",
                            Message = "Selected month and year already exist.Process aborted.",
                            Buttons = MessageBox.Button.OK,
                            Icon = MessageBox.Icon.INFO,
                            Width = 350
                        });
                ...
                else
                {
                    ...
                    Store store = X.GetCmp<Store>("employerscheme_store");
                    store.Reload();
...

        {
                ...
                if (con_ex.State == ConnectionState.Open)
                {
                    ...
                    if (srda.HasRows)
                    {
                        ...
                        using (OracleConnection conn = new OracleConnection(app.conString()))  //
                        {
                            ...
                            try
                            {
                                //check if pending list exist for employer contribution or remittance already
                                var param_k = new DynamicParameters();


                                decimal emp_num = 0;
                                param_k.Add(name: "p_ES_ID", value: remit_cont_repo.ES_Id, dbType: DbType.String, direction: ParameterDirection.Input);
                                param_k.Add(name: "p_Log_Status", value: "PENDING", dbType: DbType.String, direction: ParameterDirection.Input);
                                param_k.Add(name: "p_result", value: emp_num, dbType: DbType.Decimal, direction: ParameterDirection.Output);
                                conn.Execute(sql: "sel_remit_conlogbatchpendexist", param: param_k, commandType: CommandType.StoredProcedure);
                                ...
                                if (tot_emp > 0)
                                {
                                    ...
                                    {
                                        Title = "Error",
                                        Message = "Pending Contribution upload exists for this employer already." + Environment.NewLine + "Approve the pending employer contribution.Proces aborted",
                                        Buttons = MessageBox.Button.OK,
                                        Icon = MessageBox.Icon.ERROR,
                                        Width = 350
                                ...
                                //// Employer, Month and Year

                                //check if Employer, Month and Year list exist for employer contribution or remittance already
                                var param_kb = new DynamicParameters();
                                decimal emp_nums = 0;
                                param_kb.Add(name: "p_ES_ID", value: remit_cont_repo.ES_Id, dbType: DbType.String, direction: ParameterDirection.Input);
                                param_kb.Add(name: "p_For_Month", value: remit_cont_repo.For_Month, dbType: DbType.Int32, direction: ParameterDirection.Input);
                                param_kb.Add(name: "p_For_Year", value: remit_cont_repo.For_Year, dbType: DbType.Int32, direction: ParameterDirection.Input);
                                param_kb.Add(name: "p_count", value: emp_nums, dbType: DbType.Decimal, direction: ParameterDirection.Output);
                                conn.Execute(sql: "SEL_REMIT_CON_EXISTFORMONTH", param: param_kb, commandType: CommandType.StoredProcedure);
                                decimal tot_empp = param_kb.Get<decimal>("p_count");
                                if (tot_empp > 0)
                                {
                                    X.Mask.Hide();
                                    X.Msg.Show(new MessageBoxConfig
                                    {
                                        Title = "Error",
                                        Message = "Employer Month and Year   exist for . - " + remit_cont_repo.Employer_Name + " " + remit_cont_repo.For_Month + " " + remit_cont_repo.For_Year + Environment.NewLine + "Monthly Contribution.Proces aborted",
                                        Buttons = MessageBox.Button.OK,
                                        Icon = MessageBox.Icon.ERROR,
                                        Width = 350
                               ...
                               // string batchno = "";
                                ...
                                while (srda.Read())
                                {
                                    ...
                                    a_value += 1;
                                    Remit_Contribution_Upload_LogRepo Remit_ConRepo = new Remit_Contribution_Upload_LogRepo();
                                    ...
                                    if (srda["EmployeeId"] != DBNull.Value)
                                    {
                                        ...
                                        else
                                        {
                                            X.Mask.Hide();
                                            //error_nos += 1;
                                            //erroor_msg = erroor_msg + "Employee ID does not exist for this employer . - " + Remit_ConRepo.Employee_Id + "-" + srda["EmployeeName"] + Environment.NewLine;

                                            X.Msg.Show(new MessageBoxConfig
                                            {

                                                Title = "Error",
                                                Message = "Employee ID does not exist for this employer . - " + Remit_ConRepo.Employee_Id + "-" + srda["EmployeeName"] + Environment.NewLine,
                                                Buttons = MessageBox.Button.OK,
                                                Icon = MessageBox.Icon.ERROR,
                                                Width = 350
                                    ...
                                    if (create_new_or_justacct == 1)
                                    {
                                        ...
                                        //{
                                        //    error_nos += 1;
                                        //    erroor_msg = erroor_msg + "Fund ID not provided for ." + Remit_ConRepo.Employee_Name + Environment.NewLine;
                                        //}
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                Title = "Error",
                                                Message = "Supply Employee Name For " + Remit_ConRepo.Employee_Id,
                                                Icon = MessageBox.Icon.ERROR,
                                                Width = 350
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                Title = "Error",
                                                Message = "Employee Contribution not in correct format- " + Remit_ConRepo.Employee_Name,
                                                Buttons = MessageBox.Button.OK,
                                                Icon = MessageBox.Icon.ERROR,
                                                Width = 350
                                            });
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                Title = "Error",
                                                Message = "Employer Contribution not in correct format -" + Remit_ConRepo.Employee_Name,
                                                Buttons = MessageBox.Button.OK,
                                                Icon = MessageBox.Icon.ERROR,
                                                Width = 350
                                            });
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                Title = "Error",
                                                Message = "Employee Salary not in correct format-" + Remit_ConRepo.Employee_Name,
                                                Buttons = MessageBox.Button.OK,
                                                Icon = MessageBox.Icon.ERROR,
                                                Width = 350
                                            });
                                        ...
                                        decimal req_Con_Employee = 0;
                                        //decimal req_Con_Employer = 0;
                                        //decimal req_Con = 0;
                                        //if (p_Salary_Rate < 0 || p_Salary_Rate == 0)
                                        //{
                                        //    req_Con = Remit_ConRepo.Employee_Con + Remit_ConRepo.Employer_Con;
                                        //}
                                        //else
                                        {
                                            req_Con_Employee = decimal.Round(p_Salary_Rate / 100 * Remit_ConRepo.Employee_Salary, 2);
                                            //req_Con_Employer = decimal.Round(p_Salary_Rate / 100 * Remit_ConRepo.Employee_Salary, 2);
                                            //req_Con = req_Con_Employee + req_Con_Employer;
                                            ////req_Con_Employee = decimal.Round(p_Salary_Rate / 100 * Remit_ConRepo.Employee_Salary, 2);

                                        }
                                        
                                        decimal Rate = p_Salary_Rate;
                                        //decimal diff_amount_Employee = decimal.Round(req_Con_Employee - Remit_ConRepo.Employee_Con, 2);
                                        //decimal diff_amount_Employer = decimal.Round(req_Con_Employer - Remit_ConRepo.Employer_Con, 2);
                                        decimal diff_amount = (Remit_ConRepo.Employee_Con + Remit_ConRepo.Employer_Con) - req_Con_Employee;
                                        ...
                                        if (Employee_Scheme_Fund_ID == "NA")
                                        {
                                            ...
                                            {
                                                Title = "Error",
                                                Message = "Error with Scheme fund for . -" + Remit_ConRepo.Employee_Name,
                                                Icon = MessageBox.Icon.ERROR,
                                                Width = 350
                                        ...
                                        param_ka.Add(name: "p_contype", value: "MAIN_CONTRIBUTION", dbType: DbType.String, direction: ParameterDirection.Input);
										...
										//
										//Remit_ConRepo.Employer_Con = (Remit_ConRepo.Employer_Con);
          //                              Remit_ConRepo.Employee_Con = Math.Round(Remit_ConRepo.Employee_Con);
          //                              Remit_ConRepo.Employee_Salary = Math.Round(Remit_ConRepo.Employee_Salary, 2);
                                        ...
                                        param.Add(name: "p_Req_Con", value: req_Con_Employee, dbType: DbType.Decimal, direction: ParameterDirection.Input);
